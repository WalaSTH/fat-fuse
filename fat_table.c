...
u32 fat_table_get_next_free_cluster(fat_table table) {
  u32 fat_table_get_next_free_cluster = 2; /* First two clusters are reserved */
  while (le32_to_cpu(
...
void fat_table_print(fat_table table, u32 start_cluster, u32 end_cluster) {
  ...
  while (counter >= 0 && cur_cluster != end_cluster) {
    if (fat_table_is_EOC(
            table, le32_to_cpu(((const le32 *)table->fat_map)[cur_cluster]))) {
      printf("|[%u]EOC", cur_cluster);
    } else {
      printf("|[%u]%u", cur_cluster, ((le32 *)table->fat_map)[cur_cluster]);
    }
    cur_cluster++;
    counter--;
  }
